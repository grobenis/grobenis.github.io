...
![image-20210330150317969](2021-07-04-通过ndkd打包aar格式的SDK/image-20210330150317969.png)
...
![image-20210401170348808](2021-07-04-通过ndkd打包aar格式的SDK/image-20210401170348808.png)

#### 2 配置项目

<img src="2021-07-04-通过ndkd打包aar格式的SDK/image-20210401170459101.png" alt="image-20210401170459101" style="zoom: 67%;" />
...
![image-20210401170658032](2021-07-04-通过ndkd打包aar格式的SDK/image-20210401170658032.png)

#### 2 选择Library的模块

![image-20210401170716786](2021-07-04-通过ndkd打包aar格式的SDK/image-20210401170716786.png)

#### 3 配置该模块

![image-20210401171021523](2021-07-04-通过ndkd打包aar格式的SDK/image-20210401171021523.png)

现在建议从Android视图切换到Project视图，这样有利于我看看清楚整个项目的目录结构

![image-20210401171346953](2021-07-04-通过ndkd打包aar格式的SDK/image-20210401171346953.png)
...
![image-20210401171535574](2021-07-04-通过ndkd打包aar格式的SDK/image-20210401171535574.png)

生成的函数类如下：

![image-20210401171822235](2021-07-04-通过ndkd打包aar格式的SDK/image-20210401171822235.png)

#### 2 书写方法名

接下来，我们可以需要写类中的方法，目前只需要写函数名即可。我定义了一个用于计算链表平均值的函数，输入为一个

![image-20210401172213525](2021-07-04-通过ndkd打包aar格式的SDK/image-20210401172213525.png)
...
![image-20210401172646427](2021-07-04-通过ndkd打包aar格式的SDK/image-20210401172646427.png)

在terminal中，输入以下的命令：

![image-20210401172733132](2021-07-04-通过ndkd打包aar格式的SDK/image-20210401172733132.png)

我们可以在目录中看到文件`com_xiaomi_phonesleepsdk_PhoneSleepSDK.h`：

![image-20210401172919207](2021-07-04-通过ndkd打包aar格式的SDK/image-20210401172919207.png)



内容长这样：

![image-20210401173031982](2021-07-04-通过ndkd打包aar格式的SDK/image-20210401173031982.png)

### 3.4 建立JNI文件夹，建立main.c，实现函数，编写编译文件Android.mk

#### 1 建立JNI文件夹

![image-20210401173247121](2021-07-04-通过ndkd打包aar格式的SDK/image-20210401173247121.png)

#### 2 建立main.c文件 

main.c文件将实现前文中文件中的方法。作为JAVA转C的接口。

![image-20210401173328777](2021-07-04-通过ndkd打包aar格式的SDK/image-20210401173328777.png)

![image-20210401173501401](2021-07-04-通过ndkd打包aar格式的SDK/image-20210401173501401.png)

#### 3 实现接口函数

在实现函数之前，我们需要把生成的头文件，移动或者复制到jni文件夹

![image-20210401173600997](2021-07-04-通过ndkd打包aar格式的SDK/image-20210401173600997.png)

然后可以把自己的C库复制到jni目录下边。

![image-20210401193013539](2021-07-04-通过ndkd打包aar格式的SDK/image-20210401193013539.png)
...
![image-20210402191344975](2021-07-04-通过ndkd打包aar格式的SDK/image-20210402191344975.png)
...
![image-20210401193741347](2021-07-04-通过ndkd打包aar格式的SDK/image-20210401193741347.png)

生成的so库在如下的位置

![image-20210402191907379](2021-07-04-通过ndkd打包aar格式的SDK/image-20210402191907379.png)
...
![image-20210402192057568](2021-07-04-通过ndkd打包aar格式的SDK/image-20210402192057568.png)



运行结束之后，我们就可以在MyDemo/MySDK/build/outputs/aar/目录中找到MySDK-release.aar文件。

![image-20210402192217160](2021-07-04-通过ndkd打包aar格式的SDK/image-20210402192217160.png)

双击aar文件可以看到如下所示的目录；

![image-20210402192323837](2021-07-04-通过ndkd打包aar格式的SDK/image-20210402192323837.png)

可以发现包含了各种文件。jni文件夹下是四个so库。可供Java程序调用的so库就在jni目录中。

![image-20210402192431250](2021-07-04-通过ndkd打包aar格式的SDK/image-20210402192431250.png)
...
1. 把aar文件放在一个文件目录内，比如就放在libs目录内；
   ...
   ![image-20210402193110508](2021-07-04-通过ndkd打包aar格式的SDK/image-20210402193110508.png)
...
5. 运行程序得到正确结果
   ...
   ![image-20210419201234352](2021-07-04-通过ndkd打包aar格式的SDK/image-20210419201234352.png)
