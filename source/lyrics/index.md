...
<!-- 黄铜长条名牌:置于整页最上方(歌词屏之上),点击仍触发终章彩蛋 -->
<div class="deco-cell deco-cell-neon">
...
<div class="lyric-scene" data-scene="4" data-bg-crt="crt-06-qinghuaci.jpg">
<div class="crt-particles" data-particles="dawn"></div>
</div>
<div class="lyric-scene" data-scene="5" data-bg-crt="crt-09-yasugongshang.jpg">
<div class="crt-particles" data-particles="dawn"></div>
</div>
...
<!-- 第 4 首:青花瓷(周杰伦) - 钴蓝瓷白 -->
<div class="lyric-card" data-scene="4" data-bg-crt="crt-06-qinghuaci.jpg">
<div class="lyric-card-header">
...
<!-- 第 5 首:雅俗共赏(许嵩) - 水墨黑白 -->
<div class="lyric-card" data-scene="5" data-bg-crt="crt-09-yasugongshang.jpg">
<div class="lyric-card-header">
...
<!-- 读数窗:频道 / 曲目 / 播放状态 / 音量条(JS 联动) -->
<div class="tv-readout">
<span class="tv-ro-ch" id="tvRoCh">CH 01</span>
<span class="tv-ro-title" id="tvRoTitle">曾经的你 · 许巍</span>
<span class="tv-ro-status" id="tvRoStatus">&#9654;</span>
<span class="tv-ro-vol" id="tvRoVol" aria-label="音量"><i></i><i></i><i></i><i></i><i></i></span>
</div>

<div class="tv-panel-deck">

<div class="tv-deck-cell tv-deck-left">
...
<div class="tv-deck-cell tv-deck-right">
<span class="tv-knob-wrap">
...
<!-- 功能键条:陈列品彩蛋收编为机身按键(保留原 id,JS 按 id 绑定,交互不变) -->
<div class="fx-bar">
<button class="fx-btn" id="decoVinyl" type="button" title="黑胶唱片">黑胶</button>
<button class="fx-btn" id="decoMic" type="button" title="复古麦克风">麦克风</button>
<button class="fx-btn" id="decoCassette" type="button" title="老式磁带">磁带</button>
<button class="fx-btn" id="decoCd" type="button" title="CD 光盘">CD</button>
<button class="fx-btn" id="decoPoster" type="button" title="复古海报">海报</button>
<button class="fx-btn" id="decoCandle" type="button" title="旧蜡烛">蜡烛</button>
</div>
...
(function () {
  ...
  // 播放列表：10 个频道一一对应本地音频（文件放 source/music/ 下，文件名见数组）
  var TRACKS = [
    '/music/zengjingdeni.mp3',      // 01 曾经的你
    '/music/xiaochou.mp3',          // 02 消愁
    '/music/daoxiang.mp3',          // 03 稻香
    '/music/qinghuaci.mp3',         // 04 青花瓷
    '/music/yasugongshang.mp3'      // 05 雅俗共赏
  ];
  var curTrack = 0; /* 已挂载曲目下标；0 对应默认 src 的第一首(曾经的你) */

  // 切台切歌：切到不同频道才换 src，播放状态跨台延续（正在播则续播新歌）
  function loadTrack(sceneIdx) {
    var tIdx = sceneIdx - 1;
    if (tIdx < 0 || tIdx >= TRACKS.length) return;
    if (tIdx === curTrack) return;
    var audio = document.querySelector('audio');
    if (!audio) return;
    var wasPlaying = !audio.paused;
    curTrack = tIdx;
    audio.src = TRACKS[tIdx];
    audio.load();
    if (wasPlaying) {
      var p = audio.play();
  ...
  function applyPageScene(n) {
    ...
    if (n >= 1 && n <= 5) document.body.classList.add('page-bg-' + n);
  }
  ...
  function show(n, dir) {
    ...
    scenes.forEach(function (s) {
      var sceneOn = parseInt(s.getAttribute('data-scene'), 10) === sceneIdx;
      s.classList.toggle('is-active', sceneOn);
      if (sceneOn) ensureParticles(s);
    });

    // 同步整页背景场景
    applyPageScene(sceneIdx);

    // 同步切歌：不同频道才换音频，正在播放则切台后自动续播
    loadTrack(sceneIdx);
  ...
  /* 惰性生成：只为访问过的场景渲染粒子，避免 10×60 个 span 常驻 DOM */
  function ensureParticles(sceneEl) {
    if (!sceneEl) return;
    var host = sceneEl.querySelector('.crt-particles');
    if (!host) return;
    if (host.getAttribute('data-spawned') === '1') return;
    spawnParticles(host, host.getAttribute('data-particles'));
    host.setAttribute('data-spawned', '1');
  }
  /* 初始只渲染当前激活的场景 */
  scenes.forEach(function (s) {
    if (s.classList.contains('is-active')) ensureParticles(s);
  });
})();
  ...
  (function () {
    ...
    function endDrag(e) {
      ...
      var stopTimer = setInterval(function () {
        d.speed *= 0.92;
        d.rot += d.speed * 6;
        v.style.transform = 'rotate(' + d.rot + 'deg)';
        if (d.speed < 0.05) {
...
/* ===== 功放读数窗联动:频道/曲目/播放状态/音量条 ===== */
(function () {
  var chEl = document.getElementById('tvRoCh');
  var titleEl = document.getElementById('tvRoTitle');
  var statusEl = document.getElementById('tvRoStatus');
  var volEl = document.getElementById('tvRoVol');
  if (!chEl || !titleEl) return;
  var audio = document.querySelector('audio');
  var chNum = document.getElementById('tvChannelNum');

  /* 频道号文本变化 -> 同步读数窗(原有 show() 会改写 #tvChannelNum) */
  function syncTrack() {
    if (chNum && chNum.textContent.trim() === '11') {
      chEl.textContent = 'CH 11';
      titleEl.textContent = 'SECRET CHANNEL';
      return;
    }
    var card = document.querySelector('.lyric-card.is-active');
    if (!card) return;
    var cards = document.querySelectorAll('.lyric-card');
    var num = (Array.prototype.indexOf.call(cards, card) + 1).toString();
    if (num.length < 2) num = '0' + num;
    chEl.textContent = 'CH ' + num;
    var h3 = card.querySelector('h3');
    var singer = card.querySelector('.lyric-singer');
    titleEl.textContent = (h3 ? h3.textContent : '') + (singer ? ' · ' + singer.textContent : '');
  }
  if (chNum && window.MutationObserver) {
    new MutationObserver(syncTrack).observe(chNum, { childList: true, characterData: true, subtree: true });
  }
  syncTrack();

  /* 音量条:跟随 audio.volume(五段) */
  function renderVol() {
    if (!volEl || !audio) return;
    var n = Math.round((audio.volume || 0) * 5);
    var bars = volEl.querySelectorAll('i');
    for (var i = 0; i < bars.length; i++) bars[i].classList.toggle('is-lit', i < n);
  }
  if (audio) {
    ...
    var syncStatus = function () {
      ...
      statusEl.classList.toggle('is-playing', playing);
    };
    ['play', 'pause', 'ended'].forEach(function (evt) { audio.addEventListener(evt, syncStatus); });
    syncStatus();
  }
})();
</script>
