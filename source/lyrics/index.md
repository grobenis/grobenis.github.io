...
<!-- CD 解锁场景:两个可爱小人守着看电视 -->
<div class="cd-watch" id="cdWatch">
  <div class="cw-p cw-p-left"><span class="cw-body"></span></div>
  <div class="cw-tv">
    <span class="cw-tv-screen"></span>
    <span class="cw-tv-leg"></span>
  </div>
  <div class="cw-p cw-p-right"><span class="cw-body"></span></div>
</div>

<!-- 终章:电视机弹出 I♥你 粉色字样 -->
<div class="finale-text" id="finaleText">I <span>♥</span> 你</div>
...
(function () {
  ...
  var BALL_COLORS = ['#ff8aa0', '#ffb36b', '#ffd86b', '#9ad7ff', '#c3a6ff', '#9ef0c8'];
  ...
  function spawnBalloon() {
    var c = centerOf('decoMic');
    var bc = BALL_COLORS[Math.floor(Math.random() * BALL_COLORS.length)];
    var size = 18 + Math.random() * 14;
    var wx = (Math.random() - 0.5) * 160;
    var dur = 4 + Math.random() * 2;
    var p = document.createElement('div');
    p.style.cssText = '--bc:' + bc + ';--wx:' + wx + 'px;--fdur:' + dur + 's;' +
      'width:' + size + 'px;height:' + Math.round(size * 1.22) + 'px;' +
      'left:' + (c.x - size / 2) + 'px;top:' + c.y + 'px;';
    makeP(p, 'balloon');
    setTimeout(function () { if (p.parentNode) p.parentNode.removeChild(p); }, dur * 1000 + 500);
  }
  /* 闪光粒子:从黑胶位置向上浪漫漂浮 */
  function spawnSparks() {
    var c = centerOf('decoVinyl');
    var colors = ['#ffe9a0', '#ffd27a', '#fff6d6', '#ffc46a', '#fffbe6'];
    for (var i = 0; i < 3; i++) {
      var size = 3 + Math.random() * 5;
      var dx = (Math.random() - 0.5) * 130;
      var dy = -(110 + Math.random() * 160);
      ...
      p.style.cssText = '--cc1:' + colors[Math.floor(Math.random() * colors.length)] + ';' +
        ...
        'left:' + (c.x + (Math.random() - 0.5) * 40) + 'px;top:' + c.y + 'px;';
      makeP(p, 'spark');
  ...
  toggle('decoMic', spawnBalloon, 900);
  toggle('decoCassette', spawnPetals, 700);

  /* CD:解锁条件=黑胶/麦克风/磁带都点过一次;出现两个小人守着看电视 */
  ...
  if (cd) {
    ...
    cd.addEventListener('click', function () {
      ...
      if (!cw) return;
      cw.classList.add('is-on');
      if (cdTimer) clearTimeout(cdTimer);
      cdTimer = setTimeout(function () { cw.classList.remove('is-on'); }, 5200);
    });
  }
